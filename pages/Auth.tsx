...
const Auth: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const metadata = !isLogin ? { first_name: name, last_name: surname } : undefined;
            await signInWithMagicLink(email, metadata);
            setMessage('Sihirli link e-posta adresinize gönderildi! Lütfen kutunuzu kontrol edin.');
        } catch (error) {
                ...
                <div className="bg-surface-card rounded-2xl shadow-card p-6 w-full border border-border-subtle shrink-0">
                    ...
                    {message ? (
                        <div className="text-center py-6">
                            ...
                            <h3 className="text-lg font-semibold text-ink mb-2">E-postanızı Kontrol Edin</h3>
                            <p className="text-subtle text-sm px-4">{message}</p>
                            <button 
                                onClick={() => setMessage('')}
                        ...
                        <form className="space-y-4" onSubmit={handleSubmit}>
                            ...
                            <button 
                                className="w-full flex justify-center py-3.5 px-4 rounded-xl shadow-sm text-base font-semibold text-white bg-primary hover:bg-primary-intro-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-all active:scale-[0.98] mt-2 disabled:opacity-70 disabled:cursor-not-allowed" 
                                type="submit"
                                disabled={loading}
                            >
                                {loading ? 'Gönderiliyor...' : (isLogin ? 'Sihirli Link Gönder' : 'Kayıt Ol')}
                            </button>
                        </form>
                    )}
                    ...
                    <button 
                        onClick={async () => {
                            ...
                            try {
                                ...
                                alert("Google ile giriş yapılamadı. Lütfen Supabase ayarlarınızın doğru olduğundan emin olun." + (error.message ? `\nHata: ${error.message}` : ""));
                                setLoading(false);
                            }
                        }}
                        disabled={loading}
                        className="flex items-center justify-center w-full px-4 py-3 border border-border-subtle rounded-xl bg-paper-dark text-sm font-medium text-ink hover:bg-paper-texture transition-colors disabled:opacity-70 disabled:cursor-not-allowed" 
                        type="button"
                    >
                        {loading ? (
                            <span className="w-5 h-5 border-2 border-subtle border-t-ink rounded-full animate-spin mr-3"></span>
                        ) : (
                            <img 
                                alt="Google logo" 
